...
async def main():
    ...
    if example == "youtube" or example == "all":
        from .youtube_example import youtube_examples
        await youtube_examples()
        print("\n" + "="*50 + "\n")
    
    if example == "github" or example == "all":
        from .github_example import github_examples
        await github_examples()
        print("\n" + "="*50 + "\n")
    
    if example == "ddg" or example == "all":
        from .ddg_example import ddg_examples
        await ddg_examples()
        print("\n" + "="*50 + "\n")
    
    if example == "bs" or example == "all":
        from .beautifulsoup_example import bs_examples
        await bs_examples()
        print("\n" + "="*50 + "\n")
    
    if example == "arxiv" or example == "all":
        from .arxiv_example import arxiv_examples
        await arxiv_examples()
        print("\n" + "="*50 + "\n")
    
    if example == "combined" or example == "all":
        from .combined_example import main as combined_main
        await combined_main()
